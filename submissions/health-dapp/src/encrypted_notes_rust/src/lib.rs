...
struct CanisterState {
    ...
    notes: BTreeMap<PrincipalName, Vec<EncryptedData>>,
}
...
thread_local! {
    ...
    pub static NOTES_BY_USER: RefCell<BTreeMap<PrincipalName, Vec<EncryptedData>>> = RefCell::new(BTreeMap::new());
}
...
fn get_notes() -> Vec<EncryptedData> {
    let user = caller();
...
fn update_note(note: EncryptedData) {
    let user = caller();
